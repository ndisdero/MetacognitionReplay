# This code is a fixed version of the last code I sent Kobe.
...
CONFIDENCE_DELAY_ESTIMATE = 1.5   # estimated vividness rating duration (explanation: non-mental replay trials shorter than mental replay. This delay evens them out) 
...
def run_trial(block_type, block_number, trial_num, global_trial, gabor_direction, stim_strength, stim_duration,
    ...
    if block_type == "with_mental_replay":
        ...
        # Insert matched delay 
        matched_delay = MENTAL_REPLAY_PAUSE + CONFIDENCE_DELAY_ESTIMATE
        core.wait(matched_delay)
    ...
    for measure, prompt_text, keymap in prompts:
        ...
        while rating is None:
            ...
            prompt.draw()
            draw_visual_scale(win, selected=rating, labels=labels, y_offset=-200)
            win.flip()
        core.wait(0.5)
